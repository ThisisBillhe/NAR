...
def sample_videos(
    ...
    max_seq_length = larp_ar_model.max_seq_length + larp_ar_model.cls_token_num
    ...
    larp_ar_model.setup_diagonal_mask(
        diagonal_mask, 
        larp_ar_model.cls_token_num
    )
    diagonal_mask = diagonal_mask.unsqueeze(0).repeat(sample_batch_size, 1, 1)
    ...
    for i, data in enumerate(pbar):
        ...
        bsz = c.shape[0]
        larp_ar_model.training = True # TODO: just for utilizing training loss calculation procedure
        _, loss = larp_ar_model(
            cond_idx=c,
            idx=sampled_seqs,
            targets=sampled_seqs.to(dtype=torch.int64).contiguous(),
            mask=diagonal_mask[:bsz]
        )
        loss_list.append(loss)
        larp_ar_model.training = False
...
def main(args):
    ...
    if os.path.exists(args.ar_model):
        ...
        larp_ar_model = LARP_AR.from_checkpoint(args.ar_model, args).cuda()
    else:
